...
def printUsage():
	print "python find_best_NN.py <benchmark> <kernel name>"
	exit(1)
...
def main():
	...
	kernelName = sys.argv[2]
	nn_path = base_path + "/nn.configs/" + kernelName
	...
	if(best_rerun == 0):
		src_file = nn_path + "/" + best_topology + "/" + best_epoch + "/" + best_run + "/" + sys.argv[1] + "_" + kernelName + "_FANN_RPROP.nn"
	else:
		src_file = nn_path + "/" + best_topology + "/" + best_epoch + "/" + best_run + "/" + "rerun_" + str(best_rerun) + "/" + sys.argv[1] + "_" + kernelName + "_FANN_RPROP.nn"

	dst_file = base_path + "/fann.config/" + kernelName + ".nn"
